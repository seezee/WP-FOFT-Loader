...
class WP_FOFT_Loader {
	...
	public function admin_enqueue_scripts( $hook = '' ) {
		...
		if ( ( wpfl_fs()->is__premium_only() ) &&  ( wpfl_fs()->can_use_premium_code() )) {
			...
			wp_register_script( $this->token . '-ayshim', $this->assets_url . 'js/ays-beforeunload-shim' . $this->script_suffix . '.js', 'jquery', '1.0.18', true );
			wp_enqueue_script( $this->token . '-ayshim' );
		}
	...
	public function admin_enqueue_fa_scripts( $hook = '' ) {
		...
		if ( ( $pagenow == 'plugins.php' ) || ( $pagenow == 'general-options.php' ) ) {
			...
			// fa-coffee & fa-font. In the event we ever need to add more 
