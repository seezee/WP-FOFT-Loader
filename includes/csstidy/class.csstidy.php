 ...
 * @version 1.0
 */
function _addtoken($type, $data, $do = false) {
    ...
    for ($i = 0, $size = strlen($string); $i < $size; $i++ )
    {
        ...
        switch($this->status)
        {
            ...
            if(csstidy::istoken($string,$i))
            {
                ...
                {
                    $this->status = 'is';
                    $this->_addtoken(AT_START, $this->at);
                }
                elseif($string{$i} == ',')
                {
            ...
            else
            {
                ...
                if(!( (ctype_space($this->at{$lastpos}) || csstidy::istoken($this->at,$lastpos) && $this->at{$lastpos} == ',') && ctype_space($string{$i})))
                {
                    $this->at .= $string{$i};
                }
            }
            ...
            if(csstidy::istoken($string,$i))
            {
                ...
                {
                    $this->status = 'ip';
                    $this->_addtoken(SEL_START, $this->selector);
                    $this->added = false;
                }
                elseif($string{$i} == '}')
                {
                    $this->_addtoken(AT_END, $this->at);
                    $this->at = '';
                    $this->selector = '';
            ...
            else
            {
                ...
                if($lastpos == -1 || !( (ctype_space($this->selector{$lastpos}) || csstidy::istoken($this->selector,$lastpos) && $this->selector{$lastpos} == ',') && ctype_space($string{$i})))
                {
                    $this->selector .= $string{$i};
                }
            }
            ...
            if(csstidy::istoken($string,$i))
            {
                if(($string{$i} == ':' || $string{$i} == '=') && $this->property != '')
                {
                    ...
                    if(!$this->get_cfg('discard_invalid_properties') || csstidy::property_is_valid($this->property)) {
                        $this->_addtoken(PROPERTY, $this->property);
                    }
                }
                ...
                elseif($string{$i} == '}')
                {
                    ...
                    $this->status = 'is';
                    $this->invalid_at = false;
                    $this->_addtoken(SEL_END, $this->selector);
                    $this->selector = '';
                    $this->property = '';
                }
                elseif($string{$i} == ';')
            ...
            if(csstidy::istoken($string,$i) || $pn)
            {
                if($string{$i} == '/' && @$string{$i+1} == '*')
                {
                    $this->status = 'ic'; ++$i;
                ...
                if(($string{$i} == '}' || $string{$i} == ';' || $pn) && !empty($this->selector))
                {
                    ...
                    if((!$this->invalid_at || $this->get_cfg('preserve_css')) && (!$this->get_cfg('discard_invalid_properties') || $valid))
                    {
                        ...
                        $this->_addtoken(VALUE, $this->value);
            ...
            if($string{$i} == '*' && $string{$i+1} == '/')
            {
                $this->status = $this->from;
                $i++;
                $this->_addtoken(COMMENT, $cur_comment);
                $cur_comment = '';
            }
            else
            {
