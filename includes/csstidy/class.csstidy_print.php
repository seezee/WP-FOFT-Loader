    ...
    function _convert_raw_css()
    {
        ...
        foreach ($this->css as $medium => $val)
        {
            ...
            if ($medium != DEFAULT_AT) {
                $this->parser->_addtoken(AT_START, $medium, true);
            }

            foreach ($val as $selector => $vali)
            {
                if ($this->parser->get_cfg('sort_properties')) ksort($vali);
                $this->parser->_addtoken(SEL_START, $selector, true);

                foreach ($vali as $property => $valj)
                {
                    $this->parser->_addtoken(PROPERTY, $property, true);
                    $this->parser->_addtoken(VALUE, $valj, true);
                }

                $this->parser->_addtoken(SEL_END, $selector, true);
            }

            if ($medium != DEFAULT_AT) {
                $this->parser->_addtoken(AT_END, $medium, true);
            }
        }
    }
